...
OSLPay Merchant Portal 是一个 **B2B2C** 架构的加密货币支付商户门户平台。OSLPay 作为平台方，面向多个商户（B 端）提供支付能力，商户再面向各自的终端用户（C 端）提供加密货币购买服务。本门户服务于每个商户的业务人员和技术人员，提供从入驻申请、业务运营到技术集成的全链路支持。平台采用多租户设计，每个商户的数据完全隔离。
