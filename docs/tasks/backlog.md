...
### Phase 5: 统一入驻申请（合并 KYB + Onboarding）

> PRD: `docs/prd/15-unified-merchant-application.md`

| # | Task | 范围 | 说明 |
|---|------|------|------|
| 026a | 数据模型 + 草稿 API | 后端建表 + Entity/DTO + 保存/获取草稿 | 新建 t_merchant_application + t_application_document |
| 026b | 提交 + 状态流转 + 文件上传 | 后端提交/重提交 API + 文件上传接口 | 完整校验、状态机、文件存储 |
| 026c | 前端 Step 1-3 表单 | 前端公司信息 + 法人/UBO + 业务信息 | 草稿自动保存、UBO 动态列表 |
| 026d | 前端 Step 4-5 + 审核页 + 菜单合并 | 文件上传 UI + 确认提交 + 审核进度 + 路由调整 | 替代旧 KYB + Onboarding 页面 |
