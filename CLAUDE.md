...
7. **公司名唯一**: `t_merchant.company_name` 全局唯一，一家公司只能有一个 Merchant 记录
8. **注册**: 创建新 Merchant + 第一个 ADMIN 用户。公司名已存在则拒绝，邮箱已注册则拒绝
9. **邀请加入**: 同一 email 可存在于不同 merchant 下（UNIQUE(merchant_id, email)），但只能通过管理员邀请加入（不能通过注册）
10. **登录与商户选择**: 登录时若 email 对应多个 merchant（通过邀请加入的），需用户选择一个进入；切换商户需退出重新登录
...
1. **每个 @Test 方法必须使用 @DisplayName 注解，用中文写明测试意图**，格式：`@DisplayName("场景描述 → 预期结果")`
2. **测试方法名使用英文**，采用 `should_预期行为_when_条件` 格式
3. **测试分组**：使用 `@Nested` + `@DisplayName` 按功能模块分组
4. **覆盖维度**：每个 API 端点至少覆盖以下场景：
   - 正常流程（happy path）
   - 参数校验失败（缺失、格式错误、超长）
   - 业务规则违反（重复、不存在、状态不允许）
   - 安全场景（未认证、频率限制、信息泄漏验证）
5. **断言要求**：不仅断言 HTTP 状态码和响应 code，还要断言数据库/Redis 的最终状态
6. 前端测试同理：`describe` 和 `it` 的描述必须用中文，说明测试意图
...
class RegisterTests {
    ...
    @Test
    @DisplayName("正常注册 → 创建商户和用户，返回成功，Redis 中存在验证 token")
    void should_createMerchantAndUser_when_validRequest() { ... }

    @Test
    @DisplayName("公司名已存在 → 返回 400 统一错误消息，不泄漏具体原因")
    void should_returnGenericError_when_companyNameExists() { ... }
}
```
...
```
┌─────────────────────────────────────────────────────────────────┐
│  ① PLAN — 创建任务文件（TDD: 先测试后实现）                       │
│  梳理范围 → 先写测试用例（功能+安全） → 再写开发计划                │
│  输出: docs/tasks/current.md                                    │
│                                                                 │
│  ② REVIEW — 人工评审计划+测试用例  ⛔ 等待用户确认                 │
│  用户重点审阅: 测试用例是否覆盖安全场景、边界条件、攻击向量          │
│                                                                 │
│  ③ CODE — 编码实现（TDD 顺序）                                   │
│  先写测试代码 → 运行测试（红灯）→ 写实现代码 → 运行测试（绿灯）    │
│  每完成一个 checklist 项记录执行日志                                │
│                                                                 │
│  ④ SELF-REVIEW — 安全自查                                       │
│  对照安全检查清单逐项确认，补漏 → 更新执行日志                      │
│                                                                 │
│  ⑤ VERIFY — 人工验收             ⛔ 等待用户确认                  │
│  用户验收功能，确认符合预期                                        │
│                                                                 │
...
单个 Task 的范围必须满足以下约束：
- **后端最多 2 个新 API 端点**（不含修改现有端点）
- **前端最多 2 个新页面**
- **预估编码时间不超过 1 小时**（不含测试和文档）
- 如果超出以上任一约束，必须拆分为多个子 Task（如 003a, 003b, 003c）
- 每个子 Task 独立可验收，有自己的安全检查和测试用例

### 任务文件模板

每个任务文件必须包含以下结构。**注意 Test Cases 在 Development Plan 之前——这是 TDD 要求，先定义"怎么验证"，再定义"怎么实现"。**
...
### 安全测试
1. {未认证访问}: {操作} → {预期 HTTP 状态码和响应}
2. {频率限制}: {超出限制的操作} → {429 响应}
3. {信息泄漏}: {探测操作} → {不泄漏信息的统一响应}
4. {输入边界}: {超长/恶意输入} → {拒绝并返回错误}
...
### 安全检查清单（MANDATORY — 每个涉及 API 端点的 Task 验收前必须逐项确认）

每个新增或修改的 API 端点，提交验收前必须逐条回答以下问题。**不允许以"后续优化"为由跳过任何一项。**

- [ ] **认证**: 这个端点是否需要认证？已认证端点是否验证了 JWT？公开端点是否在 SecurityConfig 中显式放行？
- [ ] **频率限制**: 这个端点是否有 IP 级别的请求频率限制？限制阈值是否合理？
- [ ] **信息泄漏**: 错误响应是否泄漏了敏感信息（邮箱是否存在、公司是否存在）？成功/失败的响应时间是否一致（防时序攻击）？
- [ ] **输入校验**: 所有输入参数是否有类型、长度（最小+最大）、格式校验？是否防范了超长输入 DoS？
- [ ] **租户隔离**: 涉及业务数据的查询是否附加了 merchant_id 条件？
- [ ] **审计日志**: 关键操作（增删改、登录、权限变更）是否记录了结构化审计日志（含 IP、UA）？
- [ ] **HTTP 状态码**: 是否返回了正确的 HTTP 状态码（401/403/429 等），而非统一 200？
- [ ] **日志安全**: 日志中是否避免了 PII（邮箱、手机号）和安全 token 的明文输出？
- [ ] **安全响应头**: 是否配置了 CSP、HSTS、X-Frame-Options 等安全响应头？
